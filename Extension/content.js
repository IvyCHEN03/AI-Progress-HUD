(() => {
  ...
  function visibleButtonText(pattern) {
    return [...document.querySelectorAll('button, [role="button"]')].slice(-120).some(node => {
  ...
  function hasStopControl() {
    return exists(adapters.stop) || visibleButtonText(/stop|停止|中止|cancel generating|cancel response|停止生成|停止回答/i);
  }

  function computeState() {
    if (attentionVisible()) return "attention";
    if (exists(adapters.error)) return "error";
    if (hasStopControl()) return Date.now() - lastMutationAt < 1800 ? "streaming" : "thinking";
    if (generationExpected) {
      ...
      if (Date.now() - lastMutationAt < 4200) return "streaming";
      return "completed";
    }
    if (state === "completed" && Date.now() - completedAt < 45000) return "completed";
    return "idle";
  }
  ...
  function emit(force = false) {
    ...
    if (next !== state) {
      ...
      if (state === "completed") completedAt = Date.now();
